  ...
  <dialog-modal title="新建跟进记录" :visible.sync="dialogVisible" width="80%" :close-on-click-modal="false" showClose @success="handleSubmit">
    <el-form label-width="100px" size="mini">
      ...
      <el-form-item label="记录类型">
        <el-select v-model="formInfo.followRecordType">
          <el-option v-for="(item, index) in saleOption" :label="item.label" :value="item.value" :key="index"></el-option>
        </el-select>
      </el-form-item>
    </el-form>
...
<script>
  import { getFollowRecordList } from '@/api/resource/detail'
  import { saveFollowRecord } from '@/api/customer/compare'
  ...
  export default {
    ...
    data() {
      return {
        formInfo: {
          crmInfoType: 'CUSTOMER_INFO', // 资源类型 1、资源线索 2、客户
          customerId: this.$route.params.id, // 资源、客户 的关联id
          followRecordType: 'SALE_RECORD', // 记录类型 1、上门拜访  2、销售记录
          content: '' // 跟进记录json串
        },
        customerTemplateFieldVoList: [],
        dialogVisible: false,
        saleOption: [
          // { id: 1, value: 'HOME_VISIT', label: '上门拜访' },
          { id: 2, value: 'SALE_RECORD', label: '销售记录' }
        ]
      }
    },
    mounted() {
    ...
    methods: {
      handleSubmit() {
        ...
        this.formInfo.content = JSON.stringify(this.customerTemplateFieldVoList)
      ...
      _addFollowRecord(data) {
        return saveFollowRecord(data).then((res) => {
          if (res.code === 200) {
